...
規劃中的 Phase 15 OCR 架構透過明確的[提供者契約（Provider Contract）](skills/agy-ppt/docs/ocr-provider-contract.md)支援使用者自備的 OCR 提供者（Bring Your Own OCR, BYO-OCR），並為未指定自訂 OCR 的使用者規劃了開箱即用的 `agy-ppt` 預設本機提供者（Tesseract 5）。
